...
async function seed() {
  console.log("Seeding menu items, gallery, and process contents...");
  ...
  const galleryImages = [
    ...
    { orderId: 5, imageUrl: "https://images.unsplash.com/photo-1511920170033-f8396924c348?auto=format&fit=crop&q=80&w=1200", category: "Atmosphere", title: "Evening Ambiance" },
  ];

  const contents = [
    { key: "cafe_location_image", type: "IMAGE", imageUrl: "https://images.unsplash.com/photo-1554118811-1e0d58224f24?auto=format&fit=crop&q=80&w=1000" },
    { key: "cafe_location_desc", type: "TEXT", textContent: "Step into our world. A space meticulously designed for the pure appreciation of coffee, where every detail enhances the tasting experience." },
  ...
  for (const item of galleryImages) {
    const exists = await prisma.galleryImage.findFirst({ where: { title: item.title } });
    if (!exists) {
